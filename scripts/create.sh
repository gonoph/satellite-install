...
: ${HG:=RHEL7-Server}
MAC=
: ${IP:=$(host $HOST | sed 's/.*has address //')}
...
if [ "x$1" = "xclean" ] ; then
  ...
  [ -z "$HID" ] && echo "# Didn't find any RHEVM VMs named $NAME"
  exit 1
fi
...
echo "# Creating disk: disk1 for vms=$VMS_ID"
cat<< EOF >/tmp/x
...
echo "# Creating NIC for vms=$VMS_ID"
...
if [ $I -eq 0 ] ; then
	echo "# VM disk status isn't up!"
	exit 1
fi

set -e

echo "# Creating host in foreman: $HOST, mac=$MAC, ip=$IP"
...
echo "Starting VM to boot from network"
cat <<EOF > /tmp/x
